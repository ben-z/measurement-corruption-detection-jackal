...
for vel_osci_mag in $(loop_with_step -2.0 2.0 0.2); do
    for vel_osci_period in $(loop_with_step -2.0 2.0 1.0); do
        __exp_name_prefix="circle-velocity-corruption-oscillation-$vel_osci_mag-$vel_osci_period"
        ...
        generate_tembo_scenario \
            ...
            corruption /jackal_velocity_controller/odom/corruption nav_msgs/Odometry linear_vel_x oscillating $vel_osci_mag --period $vel_osci_period \
        >> $__commands_file
    done
done
