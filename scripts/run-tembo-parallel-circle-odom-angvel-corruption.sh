...
; do
    for magnitude in $(loop_with_step -2.0 2.0 0.2); do
        # step
        __exp_name_prefix="odom-angvel-corruption-bias-$magnitude-$(echo "$planner_path_profile" | slugify)"
        ...
        if [ -n "$__existing_experiments" ]; then
            ...
            generate_tembo_scenario \
                ...
                corruption /jackal_velocity_controller/odom/corruption nav_msgs/Odometry angular_vel_z step $magnitude \
            >> $__commands_file
        fi

        # ramp
        __exp_name_prefix="odom-angvel-corruption-ramp-$magnitude-$(echo "$planner_path_profile" | slugify)"
        ...
        if [ -n "$__existing_experiments" ]; then
            ...
            generate_tembo_scenario \
                ...
                corruption /jackal_velocity_controller/odom/corruption nav_msgs/Odometry angular_vel_z ramp $magnitude \
            >> $__commands_file
        fi

        # oscillating
        for period in $(loop_with_step -10.0 10.0 1.0); do
            __exp_name_prefix="odom-angvel-corruption-oscillation-$magnitude-$period-$(echo "$planner_path_profile" | slugify)"
            ...
            if [ -n "$__existing_experiments" ]; then
                ...
                generate_tembo_scenario \
                    --experiment_name "$__exp_name_prefix-\$(hostname)" \
                    --gazebo_world "empty-rate_100" \
                    corruption /jackal_velocity_controller/odom/corruption nav_msgs/Odometry angular_vel_z oscillating $magnitude --period $period \
                >> $__commands_file
            fi
        done
    done
